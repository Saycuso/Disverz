...
router.get('/discord/callback', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!tokenResponse.ok) {
            ...
            return res.status(400).json({ error: 'Discord token rejection', details: tokenData });
        }
        ...
        if (!userResponse.ok) {
            ...
            return res.status(400).json({ error: 'Discord user data rejection', details: userData });
        }
