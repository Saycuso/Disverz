...
router.patch('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const { description, tags, language, welcomeChannelId, challengeChannelId } = req.body;
        ...
        if (existingServer.ownerId !== req.userId) {
             res.status(403).json({ error: 'You do not have permission to edit this server' });
             return;
        }
        ...
        if (welcomeChannelId && welcomeChannelId !== existingServer.welcomeChannelId) {
            try {
                const discordRes = await fetch(`https://discord.com/api/v10/channels/${welcomeChannelId}/invites`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bot ${process.env.DISCORD_BOT_TOKEN}`,
                        'Content-Type': 'application/json'
